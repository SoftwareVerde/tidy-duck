...
public class MostFunctionInflater {
    ...
    public List<MostFunctionParameter> inflateMostFunctionParametersFromMostFunctionId(final long mostFunctionId) throws DatabaseException {
        final Query query = new Query(
                "SELECT name, description, parameter_index, most_type_id FROM function_parameters WHERE function_id = ?"
        );
        query.setParameter(mostFunctionId);
        ...
        final List<Row> rows = _databaseConnection.query(query);
        for (final Row row : rows) {
            final String parameterName = row.getString("name");
            final String parameterDescription = row.getString("description");
