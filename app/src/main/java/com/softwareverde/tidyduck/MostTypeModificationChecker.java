...
public class MostTypeModificationChecker {
    ...
    public List<String> checkTypeForIllegalChanges(final MostType mostType) throws DatabaseException {
        ...
        _ifNotEqualAppendError(errors, mostType.isPrimaryType(), storedType.isPrimaryType(), "Primary type flag cannot be changed on released types.");
        if (_ifNotEqualAppendError(errors, mostType.getPrimitiveType().getId(), storedType.getPrimitiveType().getId(), "Base type cannot be changed on released types.")) {
            // different base type, bail out now.
            return errors;
        }
    ...
    private void _checkArray(final List<String> errors, final MostType mostType, final MostType storedType) {
        ...
        _ifNotEqualAppendError(errors, mostType.getArrayElementType().getId(), storedType.getArrayElementType().getId(), "Array element type cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getArraySize(), storedType.getArraySize(), "Array size cannot be changed on released types.");
    }

    private void _checkBitField(final List<String> errors, final MostType mostType, final MostType storedType) {
        _ifNotEqualAppendError(errors, mostType.getBitfieldLength(), storedType.getBitfieldLength(), "BitField length cannot be changed on released types.");
    ...
    private void _checkBooleanFields(final List<String> errors, final List<BooleanField> newBooleanFields, final List<BooleanField> storedBooleanFields) {
        if (!_ifNotEqualAppendError(errors, newBooleanFields.size(), storedBooleanFields.size(), "Number of boolean fields cannot be changed on released types.")) {
            ...
            for (int i = 0; i < storedBooleanFields.size(); i++) {
                ...
                _ifNotEqualAppendError(errors, newBooleanField.getBitPosition(), storedBooleanField.getBitPosition(), "BitPosition " + i + " cannot be changed on released types.");
    ...
    private void _checkClassifiedStream(final List<String> errors, final MostType mostType, final MostType storedType) {
        _ifNotEqualAppendError(errors, mostType.getStreamMaxLength(), storedType.getStreamLength(), "Classified Stream length cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getStreamMediaType(), storedType.getStreamMediaType(), "Classified Stream media type cannot be changed on released types.");
    }
    ...
    private void _checkEnum(final List<String> errors, final MostType mostType, final MostType storedType) {
        ...
        for (int i=0; i<storedType.getEnumValues().size(); i++) {
            ...
            _ifNotEqualAppendError(errors, newEnumValue.getName(), storedEnumValue.getName(), "Enum value name cannot be changed on released types.");
            _ifNotEqualAppendError(errors, newEnumValue.getCode(), storedEnumValue.getCode(), "Enum value code cannot be changed on released types.");
            // description can change
        }
    }

    private void _checkNumber(final List<String> errors, final MostType mostType, final MostType storedType) {
        _ifNotEqualAppendError(errors, mostType.getNumberBaseType().getId(), storedType.getNumberBaseType().getId(), "Number base type cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getNumberExponent(), storedType.getNumberExponent(), "Number exponent cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getNumberRangeMinimum(), storedType.getNumberRangeMinimum(), "Number range minimum cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getNumberRangeMaximum(), storedType.getNumberRangeMaximum(), "Number range maximum cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getNumberStep(), storedType.getNumberStep(), "Number step cannot be changed on released types.");
        _ifNotEqualAppendError(errors, mostType.getNumberUnit().getId(), storedType.getNumberUnit().getId(), "Number unit cannot be changed on released types.");
    }

    private void _checkRecord(final List<String> errors, final MostType mostType, final MostType storedType) {
        // name can change
        // description can change
        _ifNotEqualAppendError(errors, mostType.getRecordSize(), mostType.getRecordSize(), "Record size cannot be changed on released types.");
        if (!_ifNotEqualAppendError(errors, mostType.getRecordFields().size(), storedType.getRecordFields().size(), "Number of record fields cannot be changed on released types.")) {
            ...
            for (int i=0; i<storedType.getRecordFields().size(); i++) {
                ...
                // name can change
                // description can change
                _ifNotEqualAppendError(errors, newRecordField.getFieldIndex(), storedRecordField.getFieldIndex(), "Record field " + i + " index cannot be changed on released types.");
                _ifNotEqualAppendError(errors, newRecordField.getFieldType().getId(), storedRecordField.getFieldType().getId(), "Record field " + i + " type cannot be changed on released types.");
            }
        }
    }

    private void _checkShortStream(final List<String> errors, final MostType mostType, final MostType storedType) {
        _ifNotEqualAppendError(errors, mostType.getStreamLength(), storedType.getStreamLength(), "Short Stream length cannot be changed on released types.");
    }
    ...
    private void _checkStream(final List<String> errors, final MostType mostType, final MostType storedType) {
        ...
        if (!_ifNotEqualAppendError(errors, mostType.getStreamCases().size(), mostType.getStreamCases().size(), "Number of stream cases cannot be changed on released types.")) {
            for (int i=0; i<storedType.getStreamCases().size(); i++) {
                ...
                _ifNotEqualAppendError(errors, newStreamCase.getStreamPositionX(), storedStreamCase.getStreamPositionX(), "Stream case " + i + " position X cannot be changed on released types.");
                _ifNotEqualAppendError(errors, newStreamCase.getStreamPositionY(), storedStreamCase.getStreamPositionY(), "Stream case " + i + " position Y cannot be changed on released types.");
    ...
    private void _checkStreamCaseParameters(final List<String> errors, final List<StreamCaseParameter> newStreamCaseParameters, final List<StreamCaseParameter> storedStreamCaseParameters) {
        if (!_ifNotEqualAppendError(errors, newStreamCaseParameters.size(), storedStreamCaseParameters.size(), "Number of stream case parameters cannot be changed on released types.")) {
            for (int i=0; i<storedStreamCaseParameters.size(); i++) {
                ...
                // name can change
                // description can change
                _ifNotEqualAppendError(errors, newStreamCaseParameter.getParameterIndex(), storedStreamCaseParameter.getParameterIndex(), "Stream case parameter index cannot be changed on released types.");
                _ifNotEqualAppendError(errors, newStreamCaseParameter.getParameterType().getId(), storedStreamCaseParameter.getParameterType().getId(), "Stream case parameter type cannot be changed on released types.");
            }
        }
    }
    ...
    private void _checkStreamCaseSignals(final List<String> errors, final List<StreamCaseSignal> newStreamCaseSignals, final List<StreamCaseSignal> storedStreamCaseSignals) {
        if (!_ifNotEqualAppendError(errors, newStreamCaseSignals.size(), storedStreamCaseSignals.size(), "Number of stream case signals cannot be changed on released types.")) {
            for (int i=0; i<storedStreamCaseSignals.size(); i++) {
                ...
                // name can change
                // description can change
                _ifNotEqualAppendError(errors, newStreamCaseSignal.getSignalIndex(), storedStreamCaseSignal.getSignalIndex(), "Stream case signal index cannot be changed on released types.");
                _ifNotEqualAppendError(errors, newStreamCaseSignal.getSignalBitLength(), storedStreamCaseSignal.getSignalBitLength(), "Stream case signal bit length cannot be changed on released types.");
            }
        }
    }

    private void _checkString(final List<String> errors, final MostType mostType, final MostType storedType) {
        _ifNotEqualAppendError(errors, mostType.getStringMaxSize(), storedType.getStringMaxSize(), "String max size cannot be changed on released types.");
    }
